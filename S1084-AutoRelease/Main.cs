                ...
                if (result == DialogResult.OK)
                {
                    ...
                    using (FileStream fs = new FileStream(path + "Software Master Part Number List.html", FileMode.Create))
                    {
                        using (StreamWriter w = new StreamWriter(fs))
                        {
                            ...
                            w.WriteLine("<br><br>");
                            w.WriteLine("<table>");
                            w.WriteLine("<tr><th>Number</th><th>Status</th><th>Platform</th><th>Short Description</th><th>Description</th></th>");

                            foreach (XmlNode Sxxxx in softwareProjects)
                            {
                                w.WriteLine("<tr>");
                                w.WriteLine("<td style=\"min-width: 200px\">" + Sxxxx.Name + "</td>");
                                w.WriteLine("<td style=\"min-width: 200px\">" + Sxxxx.Attributes["active"].Value + "</td>");
